...
export default class ProductDetails extends Component {
    ...
    handleSubmit = (event) => {
        ...
        axios
            .put(
                `http://localhost:3001/api/products/update/${this.state.productId}`,
                this.state,
                { withCredentials: true }
            )
            .then((updatedProduct) => {
                const updatedProductInfo = updatedProduct.data.product;
                this.props.getData();
    ...
    handleImageChange = (event) => {
        ...
        axios
            .patch(
                `http://localhost:3001/api/products/image/${this.props.location.state.data._id}`,
                uploadData,
                { withCredentials: true }
            )
            .then((updatedImage) => {
                ...
                this.setState({ image: updatedImage.data.product.image });
            })
            .catch((err) => console.log({ err }));
    };
    ...
    handleMultiImageChange = (event) => {
        ...
        axios
            .patch(
                `http://localhost:3001/api/products/imageArray/${this.props.location.state.data._id}`,
                uploadData,
                {
            ...
            .then((updatedImages) => {
                ...
                this.setState({
                    imageArray: updatedImages.data.product.imageArray,
                });
            })
            .catch((err) => console.log({ err }));
    };
